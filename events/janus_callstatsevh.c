...
static size_t janus_callstatsehv_write_data(void *buffer, size_t size, size_t nmemb, void *userp) {
	return size*nmemb;
}
...
static void *janus_callstatsevh_handler(void *data) {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, backend);
		...
		curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, janus_callstatsehv_write_data);
		/* Any credentials? */
